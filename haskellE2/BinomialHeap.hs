...
insertTree t ts = case ts of
  [] -> [t]
  (EmptyTree : ts') -> t : ts'
  (t' : ts') -> EmptyTree : insertTree (attach t t') ts'
...
findMin (BinomialHeap _ trees) =
  let nonEmptyTrees = [t | t@(Node prio _ _) <- trees]
      cmp (Node p1 _ _, _) (Node p2 _ _, _) = compare p1 p2
      (Node prio key _, _) = minimumBy cmp [(t, ()) | t <- nonEmptyTrees]
  in Just (prio, key)
...
mergeTrees ts1 ts2 =
    let
        mergeHelper (t1, t2) = insertTree (attach t1 t2) []
        zipped = zipExtend EmptyTree EmptyTree ts1 ts2
    in
        foldr (++) [] (map mergeHelper zipped)
